...
- [ ] **Step 7: Multi-Instance Server Pool** — Create a server instance pool that manages multiple `InputSyncerServer` instances, each on a different port. The pool should handle instance lifecycle (create, destroy, recycle) and track instance state (idle, in-match, full). _(Requirement #9)_
